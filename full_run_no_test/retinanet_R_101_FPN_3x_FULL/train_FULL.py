...
decay_LR = []
max_iter = 5000 # appears to overfit afterwards (roughly)0
...
DatasetCatalog, MetadataCatalog = register_dataset(img_dir, train_data)
...
def main():
    ...
    with open(f'{model_name}_FULL.pkl', 'wb') as file:
        pickle.dump(cfg, file, protocol = pickle.HIGHEST_PROTOCOL)

    print(f'cfg saved as {model_name}_FULL.pkl')
