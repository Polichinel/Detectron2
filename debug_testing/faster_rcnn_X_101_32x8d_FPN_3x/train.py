...
print(f"Using model {model_name}_DEBUG")
...
decay_LR = []
max_iter = 500 # appears to overfit afterwards (roughly)

print(f'running for {max_iter} iterations. Learing rate: {learning_rate}, Image per batch: {img_per_batch}')

output_dir = f"/home/projects/ku_00017/people/simpol/scripts/bodies/Detectron2/output/{model_name}_DEBUG"
...
def main():
    ...
    with open(f'{model_name}_DEBUG.pkl', 'wb') as file:
        pickle.dump(cfg, file, protocol = pickle.HIGHEST_PROTOCOL)

    print(f'cfg saved as {model_name}_DEBUG.pkl')
